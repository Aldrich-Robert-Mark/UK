...
// =================================================================================
Point Configuration::Position( const FrameTitle frame_id )
    {
    return config_origin[ frame_id ];
    }
void Configuration::Position( const FrameTitle frame_id, const Point point )
    {
...
string Configuration::ProgramImage( )
    {
    return this->config_program_image;
    }
...
void Configuration::DefaultFrames()
    ...
    pt.y = 200.0;
    config_origin[ ARCHIVE_LOCAL ] = pt;
    pt.x += 120.0;
    pt.y = 0.0;
    ...
    pt.y = 200.0;
    config_origin[ ARCHIVE_REMOTE ] = pt;
    pt.x += 120.0;
    pt.y = 0.0;
    ...
    pt.x += 120.0;
    config_size[ ARCHIVE_LOCAL ] = pt;
    config_size[ ARCHIVE_REMOTE ] = pt;
...
// =================================================================================
// Class constructor
Configuration::Configuration( const string program_name )
    {
    OS Disk;
    ...
    if( !Disk.FileExists( this->config_database ))
        ...
        if( Screen.ShowMessage( ) == 1 )
            ...
            this->config_base_path = Disk.HomeDir( );
            do
                {
                MessageScreen Dir;
                ...
                Dir.Message( message_string );
                Dir.Path( this->config_base_path );
                string chosen_directory = Dir.GetDirectory( );
                if( chosen_directory != "" )
                    {
                    this->config_base_path = chosen_directory;
                    this->config_database = this->config_base_path + slash + this->config_program_name + ".ini";
                    }
                else
                    {
    ...
    if( this->config_ok )
        ...
        if( !ConfigurationFile.Save( string( "base_path" ), this->config_base_path ))
            {
            Screen.Title( "Configuration file problem" );
